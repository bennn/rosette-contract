...
(provide (contract-out
  [build-tetra-blocks (-> symbol? real? real? real? real? real? real? real? real? real? real? tetra?)]
  [tetra-move (-> real? real? tetra? tetra?)]
  [tetra-rotate-ccw (-> tetra? tetra?)]
  [tetra-rotate-cw (-> tetra? tetra?)]
  [tetra-overlaps-blocks? (-> tetra? (listof block?) boolean?)]
  [tetra-change-color (-> tetra? symbol? tetra?)]
))

(require "data.rkt" "bset.rkt" racket/list)
