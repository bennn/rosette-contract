...
(provide
  ...
  (contract-out
    (build-random-population
     (-> natural-number/c population?))
    (population-payoffs
     (-> population? [listof payoff/c]))
    (death-birth
     (->* [population? natural-number/c] [#:random (or/c #f real?)] population?))
    (match-up*
     (-> population? natural-number/c population?))
))
...
;; =============================================================================
(require "automata.rkt" "utilities.rkt" racket/match racket/list)
...
(define population? (cons/c automaton*/c automaton*/c))
